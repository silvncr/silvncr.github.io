    ...
    version: () => {
        if (!showing_version) {
            ...
            document.getElementById('version').innerHTML = `-&nbsp;v${
                Object.keys(trophies_list).filter((_t) => trophies_list[_t])
                    .length
            }`;
...
const update_trophies = () => {
    ...
    result.forEach((div) =>
        document.getElementById('trophies').appendChild(div)
    );
        ...
        if (vault_active) {
            if (key == 'enter') {
                ...
                if (key == 'backspace') {
                    ...
                    vault_in.length < password_length &&
                    !['.', ','].includes(key) &&
                    (vault_in + key).trim()
                ) {
                    vault_in += key;
                ...
                link.innerHTML =
                    vault_in || link_text.match(/.{1,20}/g).join('<br>');
            }
